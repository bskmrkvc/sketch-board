...
function grid_hover_effect() {
    ...
    //adding the hover effect and setting the color
    grid_squares.forEach(square => {
        square.addEventListener('mouseover', function(e){
            square.getElementsByClassName.background = '#000000'
        })
    });
}
...
function grid_generator(num=8){
    for(let index=0; index < num ** 2; index++){
        ...
        grid_square.style.cssText = `height: calc(100%/${num}); width: calc(100%/${num});`
...
function reset_canvas() {
    ...
    reset_btn.addEventListener('click', function(){
        grid_squares.forEach(square => {
            square.style.background = '#f2dff2'
        });
    })
}
